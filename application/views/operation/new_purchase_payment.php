...
<?php echo form_open('operation/new_purchase_payment',array('class' => 'form-horizontal')); ?>
  <div class="form-group">
      <label  class="col-sm-4 control-label">موشتەری :</label>
      <div class="col-sm-8">
        <select class="form-control" name='customer_id' id='customer' required>
          <option></option>
        <?php $customer=$this->customer_model->select(array('type'=>'saler'));
            foreach ($customer as $item) {
            echo "<option value='$item->id'>$item->f_name $item->m_name</option>"  ;
            }
            $customer=$this->customer_model->select(array('type'=>'double'));
                foreach ($customer as $item) {
                echo "<option value='$item->id'>$item->f_name $item->m_name</option>"  ;
                }
         ?></select>
...
<?php
  $no=1;
  $total=0;
  $res=$this->purchase_payment_model->all();
  foreach ($res as $key) {
   ...
   ?>
    <tr>
      ...
      <td><?php echo $key->purchase_id; ?></td>
...
$(document).ready(function(){
  ...
  $("body").on('change', '#customer' ,function(){
      $.ajax({
        url:base_url + 'operation/load_purchase_invoice_list/' + $(this).val(),
    ...
    $.ajax({
      url:base_url + 'operation/delete_purchase_payment/' + delete_id ,
      type:'POST',
    success:function(data){
...
$('#table').on('click','#edit',function(e) {
    ...
    $.ajax({
      url:base_url + 'operation/edit_purchase_payment/' + edit_id ,
      type:'POST',
    success:function(data){
      $('#form').html(data);
      },
