<?php $payment=$this->payment_model->findbyid($this->uri->segment(3));
 echo form_open('operation/update_payment/'.$payment->id,array('class' => 'form-horizontal')); ?>
  ...
  <div class="form-group">
        ...
        <select class="form-control" name='sale_id' required>
          <option></option>
        <?php $sale=$this->sale_model->select(array('customer_id'=>$payment->customer_id));
            foreach ($sale as $item) { 
              if($item->id == $payment->sale_id) { $selected="selected"; } else { $selected=""; }
  ...
  <div class="form-group">
    <div class="col-sm-offset-4 col-sm-8">
      ...
      <?= anchor('operation/payment', 'پاشگەزبونەوە',array('class'=>'btn btn-primary')); ?>
    </div>
  </div>
<?php echo form_close (); ?>