...
<!-- Details Tab Content -->
  <div class="tab-pane" id="details">
    ...
    <div class="col-md-6 disable">
     <?php echo form_open('operation/update_purchase_detail/'.$purchase->id,array('class'=>'form-horizontal','id'=>'purchase_detail')) ?>
        ...
        <div class="col-sm-8">
          ...
          <?php $customer=$this->customer_model->all();
              foreach ($customer as $item) {
              	if ($item->id == $purchase->customer_id) {$selected="selected";}else{$selected="";}
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          <input type="text" class="form-control"  name="title" value="<?php echo $purchase->title ?>" required>
        </div>
      </div>
      <div class="form-group">
        <label  class="col-sm-4 control-label">ژمارەی فاکتوری کڕین :</label>
        <div class="col-sm-8">
          <input type="text" class="form-control"  name="invoice_no" value="<?php echo $purchase->invoice_no ?>" required>
        </div>
      </div>
      <div class="form-group">
        <label  class="col-sm-4 control-label">داشکاندن :</label>
        <div class="col-sm-8">
          <input type="text" class="form-control"  name="discount" value="<?php echo $purchase->discount ?>" required>
        </div>
      </div>
      <div class="form-group">
        <label  class="col-sm-4 control-label">رێکەوت :</label>
        <div class="col-sm-8">
          <input type="date" class="form-control"  data-date-format="YYYY-MM-DD"  name="date_time" value="<?php echo $purchase->date_time ?>" required>
        </div>
      </div>
      <div class="form-group">
        <label  class="col-sm-4 control-label">تێبینی:</label>
        <div class="col-sm-8">
          <textarea class="form-control" name="description" rows="5"><?php echo $purchase->description ?></textarea>
        </div>
      </div>
      <div class="form-group">
...
<!-- Items Tab Content -->
  ...
  <a href="<?php echo $purchase->id ?>" id="new_item" class="btn btn-primary" >زیادکردن <span class="glyphicon glyphicon-plus" aria-hidden="true" ></span></a>
  ...
  <table class="table table-striped table-condensed table-hover" id="table">
	<thead>
		...
		<th>نرخ</th>
		<th>نرخی گشتی</th>
		<th>کارگێری  <a href="<?php echo $purchase->id ?>" id="new" class="btn btn-danger" >نوێ کردنەوە <span class="glyphicon glyphicon-refresh" aria-hidden="true" ></span></a></th>
	</thead>

		<?php $no=1;
		$total=0;
		$purchase_items=$this->purchase_details_model->select(array('purchase_id'=>$purchase->id));
...
<script type="text/javascript">
  $(document).ready(function(){
   ...
   $('#purchase_detail').on('submit',function(e) {

   	  $('#purchase_detail button').addClass('disabled');
      $('#purchase_detail button').text('ناردن ...');
      $.ajax({
      ...
      success:function(data){
	      console.log(data);
	      $('#purchase_detail button').removeClass('disabled');
          $('#purchase_detail button').text('پاشەکەوت کردن');
   ...
   //submit item edit form
    $('body').on('submit','#update_purchase_item',function(e) {

   	  $('#update_purchase_item button').addClass('disabled');
      $('#update_purchase_item button').text('ناردن ...');
      $.ajax({
      url:$(this).attr("action"),
      data:$(this).serialize(),
      type:'POST',
      success:function(data){
        ...
        $('#update_purchase_item button').removeClass('disabled');
      ...
      error:function(data){
          ...
          $('#edit_modal').modal("hide");
          $('#update_purchase_item button').removeClass('disabled');
          $('#update_purchase_item button').removeClass('btn-danger');
      	  $('#item_notify').text('ئەنجامەکە سەرکەوتو نەبۆ');
      }
      });
      e.preventDefault(); //=== To Avoid Page Refresh and Fire the Event "Click"===
    });
...
$('#items').on('click','#new_item',function(e){
			...
			$('#new_content').load(base_url + 'operation/new_purchase_detail/' + sub_edit_id);
   ...
   $('#table').on('click','#edit_item',function(e){
			...
			$('#update_sub').load(base_url + 'operation/edit_purchase_detail/' + edit_id);
  ...
  $("#new_modal").on('change', '#product' ,function(){
    ...
    $.ajax({
      url:base_url + 'operation/delete_purchase_item/' + delete_id ,
      type:'POST',
    success:function(data){
      console.log(data);
